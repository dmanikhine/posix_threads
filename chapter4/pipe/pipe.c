...
/* Internal function to send a "message" to the specified pipe stage. Threads use this to pass
 * along the modified data item. */
int pipe_send (stage_t *stage, long data)
{
    int status;

    status = pthread_mutex_lock (&stage->mutex); if (status != 0) return status;
...
/*
 * The thread start routine for pipe stage threads. Each will wait for a data item passed from the
 * caller or the previous stage, modify the data and pass it along to the next (or final) stage.
 */
void *pipe_stage (void *arg)
{
     ...
     * The call to pthread_cond_wait implicitly unlocks the mutex while the thread is waiting, allowing other threads
     * to make progress. Because the loop never terminates, this function has no need to unlock the mutex explicitly.
     */
}

/* External interface to create a pipeline. All the data is initialized and the threads created. They'll
 wait for data. */
 ...
 * data to the first stage. The routine returns while the pipeline processes in parallel. Call the
 * pipe_result return to collect the final stage values (note that the pipe will stall when each stage fills,
